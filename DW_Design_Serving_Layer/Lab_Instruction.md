...
1. Step 1
    ...
    ![The manage hub is highlighted.](img/-000115.png "New container")
...
3. Copy the **Server name** value on the Overview pane.
    ...
    ![The database server name value is highlighted.](img/sqlserverurl.png)

4. Open Azure Data Studio.

5. Select **Servers** on the left-hand menu, then click **Add Connection**.

    ![The add connection button is highlighted in Azure Data Studio.](img/ads-add-connection-button.png "Add Connection")
...
6. In the Connection Details form, provide the following information:
    ...
    ![The connection details are completed as described.](img//ads-add-connection.png "Connection Details")
...
8. Select **Servers** in the left-hand menu, then right-click the SQL server you added at the beginning of the lab. Select **New Query**.
    ...
    ![The New Query link is highlighted.](img//ads-new-query.png "New Query")
...
10. Select **Run** or hit `F5` to execute the query.
    ...
    ![The query and Run button are highlighted.](img/execute-setup-query.png "Execute query")

    Now we have three dimension tables and a fact table. Together, these tables represent a star schema:

    ![The four tables are displayed.](img/star-schema-no-relationships.png "Star schema: no relationships")
...
14. Replace **and execute** the query with the following to create the `FactResellerSales` primary key and foreign key relationships:
    ...
    ![The star schema is displayed with relationship keys.](img/star-schema-relationships.png "Star schema with relationships")
...
![Sample snowflake schema.](img/snowflake-schema.png "Snowflake schema")
...
5. Replace **and execute** the query with the following to create foreign key relationships between `DimProduct` and `DimProductSubcategory`, and `DimProductSubcategory` and `DimProductCategory`:
    ...
    ![The three product tables are displayed.](img/snowflake-dimension-product-tables.png "Product snowflake dimension")
    ...
    ![The snowflake schema is shown.](img/snowflake-schema-completed.png "Snowflake schema")
...
3. Replace **and execute** the query with the following to create foreign key relationships between `DimReseller` and `DimGeography`, and `DimGeography` and `DimCustomer`:
    ...
    ![The reseller snowflake dimension is displayed.](img/snowflake-dimension-reseller.png "Reseller snowflake dimension")

    Now let us look at how these new tables add another level of detail to our snowflake schema:

    ![The finalized snowflake schema.](img/snowflake-schema-final.png "Snowflake schema")
...
3. Replace **and execute** the query with the following to create foreign key relationships between `FactRetailerSales` and `DimDate`:
    ...
    ![The time dimension table is highlighted in the snowflake schema.](img/snowflake-schema-time-dimension.png "Time dimension added to snowflake schema")
...
2. Replace **and execute** the query with the following to view the time dimension table data:
    ...
    ![The time dimension table output is displayed.](img/time-dimension-table-output.png "Time dimension table output")
...
1. Paste **and execute** the following query to retrieve reseller sales data from the snowflake schema at the reseller, product, and month granularity:
    ...
    ![The reseller query results are displayed.](img/reseller-query-results.png "Reseller query results")
...
2. Replace **and execute** the query with the following to limit the results to October sales between the 2012 and 2013 fiscal years:
    ...
    ![The query results are displayed in a table.](img/reseller-query-results-date-filter.png "Reseller query results with date filter")
...
2. Open the resource group for this lab, then select the **Synapse workspace**.
    ...
    ![The workspace is highlighted in the resource group.](img/rg-synapse-workspace.png "Synapse workspace")

3. In your Synapse workspace Overview blade, select the **Open** link within `Open Synapse Studio`.

    ![The Open link is highlighted.](img/open-synapse-studio.png "Open Synapse Studio")
...
1. Paste **and execute** the following query to retrieve reseller sales data from the Synapse star schema at the reseller location, product, and month granularity:
    ...
    ![The reseller query results are displayed.](img/reseller-query-results-synapse.png "Reseller query results")
...
2. Replace **and execute** the query with the following to limit the results to October sales between the 2012 and 2013 fiscal years:
    ...
    ![The query results are displayed in a table.](img/reseller-query-results-date-filter-synapse.png "Reseller query results with date filter")
